...
async function insertTeacherSchedule(obj) {
    try {
        ...
        if (obj.name) {
            const schedule = await postData(dbServer, '/read/readOne/TeacherSchedule', { condition: { Disabled: 0 } })
            if (!schedule.data[0]) {
                const ans = await postData(dbServer, '/create/insert', { entity: MONGO_TEACHERSCHEDULE_COLLECTION, ...obj })
                return ans.data;
            }
        }
...
async function findTeacherScheduleByTeacherName(name) {
    try {
        const ans = await postData(dbServer, '/read/readMany/Teachers', { condition: { TecherName: `'${name}'`, Disabled: 0 } })
        if (id.data[0]) {
